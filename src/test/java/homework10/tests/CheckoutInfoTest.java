...
import homework10.factory.CheckoutDataFactory;
...
//        скриншота при падении как указано в условии TestListener
    ...
    //НОВЫЙ ТЕСТ С FAKER
    @Test(description = "Успешное заполнение формы с рандомными данными (Faker)",
            testName = "Успешное заполнение формы с Faker",
            priority = 5)
    public void successfulCheckoutWithRandomData() {
        addCartToCheckoutInfo();

        String firstName = CheckoutDataFactory.getRandomFirstName();
        String lastName = CheckoutDataFactory.getRandomLastName();
        String postalCode = CheckoutDataFactory.getRandomPostalCode();
