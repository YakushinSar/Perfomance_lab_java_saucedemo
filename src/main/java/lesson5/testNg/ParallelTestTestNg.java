...
Проблема без ThreadLocal:
// Без ThreadLocal — один driver на все потоки
public class BaseTest {
    static WebDriver driver;  // static — общий для всех потоков!

    @Test
    public void test1() {
        driver = new ChromeDriver(); // Поток 1 создаёт драйвер
        // Поток 2 в этот момент может закрыть этот же драйвер!
    }
}

Как ThreadLocal решает проблему:
// С ThreadLocal — у каждого потока свой driver
public class DriverManager {
    private static final ThreadLocal<WebDriver> driverThreadLocal = new ThreadLocal<>();

    public static WebDriver getDriver() {
        return driverThreadLocal.get();  // возвращает driver текущего потока
    }

    public static void setDriver(WebDriver driver) {
        driverThreadLocal.set(driver);   // сохраняет driver для текущего потока
    }
}
