...
-Логи уже есть в PageObject'ах и Wrappers (они описывают каждое действие. Они не знают, хороший это тест или плохой. Их
 задача — залогировать, что они сделали.)
-TestListener/Retry— знают результат теста (упал/прошёл/скипнут/ретряется). Поэтому они могут ставить WARN или ERROR.
...
Фигурные скобки {} — это "места" для подстановки значений переменных, которые передаются в метод log.info() (или log.warn(), log.error()).
Пример с пояснением:
public void login(String username, String password) {  // ← переменные метода
    log.info("User: {}, Password: {}", username, password);  // ← подставляются в {}
    //               ↑            ↑        ↑            ↑
    //            скобка 1     скобка 2  аргумент 1   аргумент 2
}
Пояснение по наличию, отсутствию фигурных скобок
Нет аргументов -> log.info("Текст без переменных");
1 аргумент -> log.info("User: {}", username);
2 аргумента -> log.info("User: {}, Password: {}", user, pass);
3 аргумента -> log.info("X: {}, Y: {}, Z: {}", x, y, z);

# Как правильно составлять сообщение для лога (нейминг)
Структура сообщения:
Действие + (опционально) "с данными:" + переменные + (опционально) "результат"
Ситуация	        Уровень	 Пример сообщения
Открытие страницы	 INFO	"Opening login page: {}"
Ввод данных	         INFO	"Filling field '{}' with value '{}'"
Клик	             INFO	"Clicking '{}' button"
Ожидание загрузки	 INFO	"Waiting for {} page to load"
Перезапуск теста	 WARN	"Retrying test '{}', attempt {}/{}"
Тест пропущен	     WARN	"Skipping test '{}'"
Тест упал	         ERROR	"Test '{}' failed after {} attempts"
